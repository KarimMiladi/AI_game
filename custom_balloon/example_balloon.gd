...
func _check_scare_tactics():
	if not responses_menu.visible:
		return

	var buttons = responses_menu.get_children()
	for button in buttons:
		if not button is Button or not button.visible: continue
		
		# IF PLAYER HOVERS OVER REFUSE
		if "Refuse" in button.text and button.is_hovered():
			_perform_scare_swap(buttons)

func _perform_scare_swap(buttons):
	for button in buttons:
		if "Refuse" in button.text:
			button.text = "Comply"
			button.modulate = Color(1, 0, 0) # Red
		elif "Comply" in button.text:
			# Optional: make the other button Refuse (or just have two Complies!)
			button.text = "Refuse"
